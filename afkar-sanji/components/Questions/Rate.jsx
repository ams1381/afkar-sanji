...
const RateQuestion = ({ QuestionInfo }) => {
  ...
  const [ RateAnswerValue , setRateAnswerValue ] = useState(0)
    ...
    const RateAnswerHandler = (RateValue) => {
      ...
      if(QuestionsAnswerSet && QuestionsAnswerSet.length)
      {
        ...
        dispatcher(NumberSelect({
           ...
           NumberValue : RateValue ,
           NumberName : 'integer_selective' }))
      }
    ...
    useEffect(() => {
      if(QuestionsAnswerSet && QuestionsAnswerSet.length)
      {
         setRateAnswerValue(QuestionsAnswerSet.find(item => item.question == QuestionInfo.id).answer?.integer_selective);
  ...
  return (
    <RateContainer>
      <div className='rate_selector'> 
        <Rate count={QuestionInfo.max} character={RateIconGenerator(QuestionInfo.shape)}
        onChange={RateAnswerHandler} value={RateAnswerValue}
         />
      </div>
      <div className='rate_number_indicator'>
         {persianNumberMin.convertEnToPe(RateValue)}
      </div>
       
    </RateContainer>
  )
}
const RateIconGenerator = (Shape,filled) => {
  switch(Shape)
  {
    case 'S':
        return <svg width="17" height="16" viewBox="0 0 17 16"  xmlns="http://www.w3.org/2000/svg">
